...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Gestion de la progression avec débounce
  async function trackProgress() {
    const activeSection = document.querySelector("section.active");
    if (
      !activeSection ||
      !activeSection.classList.contains("chapter") ||
      activeSection.id === "favorites" ||
      activeSection.id === "table-of-contents" ||
  ...
  window.addEventListener("scroll", () => {
    ...
    debounceTimeout = setTimeout(trackProgress, 500);
  });
  ...
  if (icon && box) {
    icon.addEventListener("click", () => {
      console.log("Clic icône assistant IA");
      isVisible = !isVisible;
    ...
    if (sendBtn && input && messages) {
      sendBtn.addEventListener("click", () => {
        ...
        if (message) {
          ...
          input.value = "";
          // Simuler une réponse (remplacez par une vraie API si nécessaire)
          const response = document.createElement("div");
          response.textContent = `Assistant: Réponse à "${message}" (implémentation de test)`;
